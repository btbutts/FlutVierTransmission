...
let pX = $state(0), pY = $state(0), pW = $state(0), pH = $state(0);
...
$effect(() => {
  if (!aggData.dl.length) { _latestMaxV = 1; return; }
...
const live = $derived.by(() => {
  ...
  return { dl: lat?.download ?? 0, ul: lat?.upload ?? 0, ts: null as number | null, isHover: false };
});
...
function buildPaths(W: number, H: number, anim: number) {
  ...
  const mt = GRAPH_MT, mb = GRAPH_MB, ch = H - mt - mb; // generous top margin prevents clipping
  const base = H - mb;
  const xOff = anim * ppx;
  ...
  return {
    dlLine: curvePath(dlP), ulLine: curvePath(ulP),
    dlArea: areaOf(dlP), ulArea: areaOf(ulP),
...
function getCH(W: number, H: number, anim: number) {
  ...
  const n = dl.length - 1;
  const ppx = W / Math.max(1, n);
  const mt = GRAPH_MT, mb = GRAPH_MB, ch = H - mt - mb;
  const yOf = (v: number) => mt + ch * (1 - Math.min(v, displayMaxV) / Math.max(displayMaxV, 1));
...
function nonPassiveWheel(node: HTMLElement, handler: (e: WheelEvent) => void) {
  ...
  return {
    update: (f: (e: WheelEvent) => void) => { h = f; },
...
function onEnter() {
  ...
  hintTimer = setTimeout(() => { showHint = true; }, HINT_DELAY);
}
...
function onWheel(e: WheelEvent) {
  ...
  if (e.ctrlKey) {
    ...
    if (e.deltaY !== 0) {
      timeWin = Math.max(MIN_WIN, Math.min(MAX_WIN, Math.round(timeWin * (e.deltaY > 0 ? 1.15 : 0.87))));
    }
...
function closeModal() {
  ...
  const r = containerEl.getBoundingClientRect();
  pX = r.left; pY = r.top; pW = r.width; pH = r.height;
  setTimeout(() => { showPortal = false; phase = 'closed'; }, ANIM_MS);
}
...
>
  ...
  <div class="px-6 text-xs">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-0.5">
        ...
        <span class="text-sm font-semibold text-blue-500 dark:text-blue-400">{fmtBps(live.dl)[0]}</span>
        <span class="text-blue-400/60 dark:text-blue-500/60">{fmtBps(live.dl)[1]}</span>
      </div>
      <!-- Upload: [value] [unit] [icon] -->
      <div class="flex items-center gap-0.5">
        <span class="text-sm font-semibold text-purple-500 dark:text-purple-400">{fmtBps(live.ul)[0]}</span>
    ...
    <div class="mb-1 mt-0.5 h-3.5 text-center leading-none" aria-live="polite">
      ...
      >
        ...
        {#if P}
          <g clip-path="url(#bw-clip-sb)">
            ...
            <path d={P.dlLine} fill="none" stroke="#3b82f6" stroke-width="1.5" stroke-opacity="0.85" />
            <path d={P.ulLine} fill="none" stroke="#a855f7" stroke-width="1.5" stroke-opacity="0.85" />
            {#if CH}
              <line
                x1={CH.x} y1={0} x2={CH.x} y2={GRAPH_H}
          ...
          <text x={svgCtnrW - 2} y="10" text-anchor="end" fill="rgba(156,163,175,0.5)" font-size="8">{maxLabel[0]} {maxLabel[1]}</text>
        {:else}
          <text
            x={svgCtnrW / 2}
            y={GRAPH_H / 2}
  ...
  {#if altSpeedOn}
    <div class="mt-1 flex justify-center px-6">
      <span class="rounded bg-blue-100 px-1.5 py-0.5 text-xs text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
        Alt Speed Active
      </span>
    </div>
  {/if}
  ...
  <div
    ...
    aria-hidden="true"
  >
    Hold Control + scroll to zoom<br />Scroll left / right to pan history<br />Double-click to expand
  </div>
</div>
...
<div class="flex h-[130px] w-full items-center justify-center gap-2 text-xs text-ColorPalette-text-quarternary">
  <ArcSpinner class="h-4 w-4" />
  <span>Loading</span>
</div>
{/if}
...
{#if showPortal}
  ...
  <div
    ...
    tabindex="-1"
    style="position: fixed; inset: 0; z-index: 9999; pointer-events: {phase === 'open' ? 'all' : 'none'};"
  ...
  >
    ...
    <div
      ...
      style="
        ...
        transition: {noTransition ? 'none' : `left ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1), top ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1), width ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1), height ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1), opacity ${ANIM_MS}ms`};
      "
    >
        ...
        <div class="flex items-center gap-3 text-sm">
          ...
          <div class="flex shrink-0 items-center gap-2">
            ...
            <div class="flex w-[88px] shrink-0 items-center gap-0.5">
              <ArrowDownBox class="h-3.5 w-3.5 shrink-0 text-blue-500 dark:text-blue-400" />
              <span class="tabular-nums font-semibold text-blue-500 dark:text-blue-400">{fmtBps(live.dl)[0]}</span>
              <span class="text-xs text-blue-400/60 dark:text-blue-500/60">{fmtBps(live.dl)[1]}</span>
            </div>
            <!-- Upload: [value] [unit] [icon] -->
            <div class="flex w-[88px] shrink-0 items-center gap-0.5">
              <span class="tabular-nums font-semibold text-purple-500 dark:text-purple-400">{fmtBps(live.ul)[0]}</span>
              <span class="text-xs text-purple-400/60 dark:text-purple-500/60">{fmtBps(live.ul)[1]}</span>
              <ArrowUpBox class="h-3.5 w-3.5 shrink-0 text-purple-500 dark:text-purple-400" />
            </div>
          </div>
        </div>
      ...
      {#if phase === 'open'}
        ...
        >
              ...
              >
                ...
                {#if P}
                  <g clip-path="url(#bw-clip-mo)">
                    ...
                    <path d={P.dlLine} fill="none" stroke="#3b82f6" stroke-width="2" stroke-opacity="0.85" />
                    <path d={P.ulLine} fill="none" stroke="#a855f7" stroke-width="2" stroke-opacity="0.85" />
                    {#if CH}
                      <line
                        x1={CH.x} y1={0} x2={CH.x} y2={mH}
                  ...
                  {@const maxLabel = fmtBps(rawMax)}
                  <text x={modalCtnrW - 4} y="14" text-anchor="end" fill="rgba(156,163,175,0.5)" font-size="9">{maxLabel[0]} {maxLabel[1]}</text>
                  <text x={modalCtnrW - 4} y={mH - 4} text-anchor="end" fill="rgba(156,163,175,0.4)" font-size="9">0</text>
                {:else}
                  <text
                    x={modalCtnrW / 2}
                    y={mH / 2}
          ...
          >
            ...
            <span class="italic">Hold Control + scroll to zoom ({fmtWin(timeWin)} window) &bull; Scroll left / right to pan history</span>
          </div>

          <!-- Alt speed badge (only when enabled) -->
          {#if altSpeedOn}
            <div class="flex justify-center pb-3">
              <span class="rounded bg-blue-100 px-2 py-0.5 text-xs text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                Alt Speed Active
              </span>
            </div>
          {/if}

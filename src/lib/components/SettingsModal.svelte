...
// ── PollService setup (shown in UI tab when server is not detected) ───────────
...
// Fetch release info lazily the first time the user is on the UI tab without a server.
$effect(() => {
  if (
    activeTab === 'ui' &&
    !$serverAvailable &&
              ...
              {:else}
                <div class="space-y-4">
                  <p class="text-ColorPalette-text-tertiary text-sm">
                    Install the optional FlutVierTransmission PollService companion on the server
                    hosting Transmission and this Web UI to enable bandwidth history persistence
                    across page reloads and server-side torrent polling.
                  </p>
                  {#if releaseLoading}
                  ...
                  {:else if releaseInfo}
                    <div class="space-y-3">
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <span class="text-ColorPalette-text-tertiary text-xs"
                          >{releaseInfo.version}</span
                        >
                      </div>
                  ...
                  >
                    <RefreshSpinner bind:this={pollServiceSpinner} loading={pollCheckLoading} class="h-4 w-4" />

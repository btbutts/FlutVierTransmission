...
// These comments are intentionally absent from the template so anyone
// editing the template are not confused by them.
// The replace line uses regex to ensure it successfully works on
//  Windows and POSIX line endings (\n or \r\n).
const SHEBANG = '#!/usr/bin/env bash';
const SHEBANG_REGEX = new RegExp(SHEBANG.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\r?\\n');
const GENERATED_HEADER = [
  '# GENERATED FILE — do not edit this file directly.',
  '# Source: PollService/templates/PollService-install-template.sh'
].join('\n');
template = template.replace(SHEBANG_REGEX, `${SHEBANG}\n${GENERATED_HEADER}\n`);

// Remove the "Regenerate" comment line, which is only relevant in the template file.
const REGENERATE_NOTICE =
  '# Regenerate: npm run build:release   (or: node build-scripts/generate-install.mjs)';
template = template.replace(
  new RegExp(REGENERATE_NOTICE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\r?\\n'),
  ''
);
